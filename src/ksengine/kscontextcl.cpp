...
bool KSContextCL::create()
{
    ...
    if( candidates.empty() ) {
        kDebug() << "Could not find any candidate platforms!";
        return false;
    }
    ...
    m_context = cl::Context(devices, properties, nullptr, nullptr, &err);
    m_device = devices.front();
    m_queue = cl::CommandQueue(m_context, m_device);

    if( err != CL_SUCCESS ) {
        kDebug() << "Could not create OpenCL context, error" << err;
        return false;
    } else {
    ...
    if( !sourcefile.open( QIODevice::ReadOnly | QIODevice::Text ) ) {
        kDebug() << "Can't open the kernel sources";
        return false;
    }
    ...
    m_program = cl::Program(m_context,sources,&err);
    if( err != CL_SUCCESS ) {
        kDebug() << "Dying with error" << err << "in cl::Program";
        return false;
    }
    err = m_program.build( devices, "-cl-std=CL1.1" );
    if( err != CL_SUCCESS ) {
        kDebug() << "Failed to compile the kernel!";
        return false;
    }

    m_kernel_applyMatrix = cl::Kernel(m_program,"applyMatrix",&err);
    if( err != CL_SUCCESS ) {
